...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# DCC-D-D — hvad der er ændret i forhold til drows original
#
# Originalen er uændret bortset fra tre steder. Alt nyt står i blokke mærket
# "DCC-D-D" og kan fjernes uden at røre resten.
#
#   1. get_opts       fire nye indstillinger, og add_stairs sat til 0
#   2. create_dungeon kalder dcc_entrance, dcc_boss_exit og dcc_xp
#   3. nederst        de nye subs
#
# Hvad de gør:
#
#   Trappen ned står inde i et rum med mindst to døre, ikke for enden af en
#   blindgang. Originalens emplace_stairs sætter altid trapper i en
#   blindgyde — check_tunnel kræver netop syv vægge omkring — så man kunne
#   blive trængt op i et hjørne ved udgangen.
#
#   Rummet er bossrummet, valgt som det rum der ligger længst fra indgangen
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# DCC-D-D: XP der skal til for at stige ét level, pr. spiller.
# Tallene er forskellene i D&D 5e's XP-tabel: 300, 900, 2700, 6500 ...
...
sub get_opts {
  my $opts = {
    ...
    'add_stairs'        => 0,           # DCC-D-D: trapperne sættes af dcc_*
    'map_style'         => 'Standard',
    'cell_size'         => 18,          # pixels
    'grid'              => 'Square',
...
sub create_dungeon {
  ...
  # - - - DCC-D-D - - -
  # Indgangen sættes før oprydningen, fordi den bruger blindgyder, som
  # remove_deadends ellers spiser. collapse_tunnels springer trappefelter
  # over, så den overlever.
  $dungeon = &dcc_entrance($dungeon);

  $dungeon = &clean_dungeon($dungeon);

  # - - - DCC-D-D - - -
  # Boss og udgang sættes efter oprydningen, fordi fix_doors først dér har
...
# DCC-D-D: indgang
#
# Op-trappen sættes med originalens blindgyde-logik. Den ende er der ikke
# noget galt med — det er dér man kom fra.
...
sub dcc_entrance {
  ...
  my @list = &stair_ends($dungeon);
     return $dungeon unless (@list);
  my $cell = $dungeon->{'cell'};
  my $stair = $list[int(rand(@list))];

  $cell->[$stair->{'row'}][$stair->{'col'}] |= $STAIR_UP;
  $cell->[$stair->{'row'}][$stair->{'col'}] |= (ord('u') << 24);
  $stair->{'key'} = 'up';
  push(@{ $dungeon->{'stair'} },$stair);
  $dungeon->{'dcc_entrance'} = $stair;

  return $dungeon;
}

# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# DCC-D-D: afstand i skridt fra ét felt til alle andre åbne felter
...
sub dcc_bfs {
  ...
  while (@queue) {
    ...
    my $dir; foreach $dir (@dj_dirs) {
      my $nr = $r + $di->{$dir};
      my $nc = $c + $dj->{$dir};

      next if ($nr < 0 || $nr > $dungeon->{'n_rows'});
      next if ($nc < 0 || $nc > $dungeon->{'n_cols'});
      next unless ($cell->[$nr][$nc] & $OPENSPACE);
      next if (defined $dist->[$nr][$nc]);
...
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
# DCC-D-D: bossrum og trappen ned
#
# Bossrummet er det rum der ligger længst fra indgangen og har mindst to
# døre. De to døre er hele pointen: trappen ned står inde i rummet, så der
# er altid mere end én vej ud af udgangen.
#
# Trappen lægges i det felt i rummet der ligger længst fra indgangen, så
# man skal tværs gennem bossrummet for at komme ned.
...
sub dcc_boss_exit {
  ...
  my $dist = $entrance
    ? &dcc_bfs($dungeon,$entrance->{'row'},$entrance->{'col'})
    : undef;

  # - - - vælg rummet - - -

  my ($boss,$boss_doors,$boss_dist) = (undef,0,-1);
  my $fallback;
  ...
  my $id; for ($id = 1; $id <= $dungeon->{'n_rooms'}; $id++) {
    ...
    if ($d > $boss_dist) {
      ($boss,$boss_doors,$boss_dist) = ($room,$doors,$d);
    }
  }
  unless ($boss) {
  ...
  $dungeon->{'dcc_boss'} = { 'id' => $boss->{'id'}, 'doors' => $boss_doors };

  # - - - læg trappen i den fjerneste ende af rummet - - -

  my ($sr,$sc,$best) = ($boss->{'north'},$boss->{'west'},-1);

  my $r; for ($r = $boss->{'north'}; $r <= $boss->{'south'}; $r++) {
    my $c; for ($c = $boss->{'west'}; $c <= $boss->{'east'}; $c++) {
      next if ($cell->[$r][$c] & $STAIRS);
      my $d = (defined $dist && defined $dist->[$r][$c]) ? $dist->[$r][$c] : 0;
      if ($d > $best) { ($sr,$sc,$best) = ($r,$c,$d); }
    }
  }
  $cell->[$sr][$sc] &= ~ 0xFF000000;          # ryd rummets talmærke i feltet
  $cell->[$sr][$sc] |= $STAIR_DN;
  $cell->[$sr][$sc] |= (ord('d') << 24);

  # image_stairs tegner trinnene fra row/col mod next_row/next_col. Peg mod
  # et nabofelt inde i rummet, så trappen vender ind i rummet.
  my $nr = ($sr > $boss->{'north'}) ? $sr - 1 : $sr + 1;
  push(@{ $dungeon->{'stair'} }, {
    'row' => $sr, 'col' => $sc,
    'next_row' => $nr, 'next_col' => $sc,
    'key' => 'down',
  });
  $dungeon->{'dcc_exit'} = { 'row' => $sr, 'col' => $sc };
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# DCC-D-D: hvor langt et rum ligger fra indgangen

sub dcc_room_dist {
  my ($dungeon,$room,$dist) = @_;
     return 0 unless ($dist);
  my $r = int(($room->{'north'} + $room->{'south'}) / 2);
...
# DCC-D-D: XP-budget
#
# Etagen skal rumme nok XP til at holdet stiger ét level, plus slack så de
# ikke behøver støvsuge hvert rum. Bossrummet tager sin andel, resten deles
# mellem rummene efter areal — et stort rum er en stor kamp.
...
sub dcc_report {
  ...
  printf("             = %d XP for holdet, x %.1f slack = %d XP på etagen\n",
    $xp->{'need_party'}, $dungeon->{'xp_slack'}, $xp->{'floor'});
